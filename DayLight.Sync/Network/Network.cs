...
public sealed class Network : IDisposable
{
    ...
    public async ValueTask SendAsync(PluginMessage data, CancellationToken cancellationToken)
    {
        ...
        {
            DayLightSyncPlugin.NetworkCancellationTokenSource.Cancel();
            Dispose();
    ...
    {
        await DayLightSyncPlugin.Instance.Network.SendAsync(data);
    }
    public async Task ReplyLine(MessageType requestType, object? data = null, string nickname = "")
    {
        ...
        await DayLightSyncPlugin.Instance.Network.SendAsync(pluginSender);
    }
}
