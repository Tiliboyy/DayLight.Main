...
public class Link : CustomCommand
{
    ...
    protected override void Respond(ArraySegment<string> arguments, Player player, ref CommandResult response)
    {
        ...
        if (LinkUtils.IsSteam64Linked(iResult))
        {
    ...
    public static string GenerateLinkCode(ulong steam64ID)
    {
        ...
        while (LinkUtils.TemporaryCodes.Select(x => x.Code).Contains(Code))
            Code = GenerateCode();
        LinkUtils.TemporaryCodes.Add(new LinkClass(steam64ID, 60, Code));
    ...
    {
        return LinkUtils.TemporaryCodes.Count(x => x.Steam64ID == steamid) >= 1;
    }
    ...
    public static IEnumerator<float> Timer()
    {
        for (;;)
        {
            ...
            foreach (var pair in LinkUtils.TemporaryCodes)
            {
                pair.Time -= 1;
                if (pair.Time == 0) LinkUtils.TemporaryCodes.Remove(pair);
            }
        }
    }
