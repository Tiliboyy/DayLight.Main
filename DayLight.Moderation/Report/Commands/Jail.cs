...
using System.Linq;
using UnityEngine;
using Logger = DayLight.Core.API.Logger;
...
public class Jail
{
    ...
    public static IEnumerator<float> JailPlayer(Player player, Vector3 Position = default)
    {
        ...
        if (JailedPlayers.TryGetValue(player, out var jailedPlayer))
        {
            ...
            {
                Logger.Error($"{nameof(jailedPlayer)}: {e}");
            }
